...
//                var PacPath = Bundle.main.path(forResource: "text", ofType: "pac")
//                print(PacPath)
//
//                PacPath = "http://192.168.1.189/test.pac"
...
//                            row.value =  URL(string: PacPath!)
			...
			<<< IntRow { row in
				...
				row.add(rule: RuleSmallerOrEqualThan(max: 86400, msg: NSLocalizedString("PAC max age must smaller than or equal to 86400.", comment: "")))
                        ...
                        <<< SwitchRow { row in
                            row.tag = "Set VPN Gloab"
                            row.title = NSLocalizedString("Set VPN Gloab", comment: "")
                            row.value = mainKeychain["general_hide_vpn_gloab"] == "true"
                        }
			...
			<<< ButtonRow { row in
				...
				let generalHideVPNIcon = (self.form.rowBy(tag: "General - Hide VPN Icon") as! SwitchRow).value!
                                ...
                                let generalVpnGloab = (self.form.rowBy(tag: "Set VPN Gloab") as! SwitchRow).value!
...
//                                                                                            providerManager.connection.stopVPNTunnel()
//                                                                                      print("dissconnect vpn!!")
//                                                                                    }
                                                                                    
                                                                                    if providerManager.connection.status == NEVPNStatus.connected {
